...
import { getUsersSearch } from '@/app/api/users'
...
export const CommandSearch = () => {
  ...
  const [movies, tvSeries, people] = [
    data?.results.filter((result) => result.media_type === 'movie') as MovieWithMediaType[],

    data?.results.filter((result) => result.media_type === 'tv') as TvSerieWithMediaType[],

    data?.results.filter((result) => result.media_type === 'person') as PersonWithMediaType[],
  ]
            ...
            {isLoading ||
              (isLoadingUsers && (
                <div className="space-y-8">
                  ...
                  <CommandSearchGroup heading={dictionary.sidebar_search.tv_series}>
                    {Array.from({ length: 5 }).map((_) => (
                      <CommandSearchSkeleton key={v4()} />
                    ))}
                  </CommandSearchGroup>
            ...
            {hasResults ? (
              <div className="">
                {hasMovies && (
                  <CommandSearchGroup heading={dictionary.movies}>
                    {movies?.map((movie) => (
                      <CommandSearchMovie item={movie} language={language} key={movie.id} />
                    ))}
                  </CommandSearchGroup>
                )}
                ...
                {hasTvSeries && (
                  <CommandSearchGroup heading={dictionary.sidebar_search.tv_series}>
                    {tvSeries?.map((tvSerie) => (
                      <CommandSearchTvSerie item={tvSerie} language={language} key={tvSerie.id} />
                    ))}
                  </CommandSearchGroup>
                )}
                ...
                {hasPeople && (
                  <CommandSearchGroup heading={dictionary.people}>
                    {people?.map((person) => (
                      <CommandSearchPerson item={person} language={language} key={person.id} />
                    ))}
                  </CommandSearchGroup>
                )}
                ...
                {hasUsers && (
                  <CommandSearchGroup heading={dictionary.users}>
                    {users?.map((user) => (
                      <CommandSearchUser item={user} language={language} key={user.id} />
                    ))}
                  </CommandSearchGroup>
                )}
              </div>
            ) : (
              <p className="p-8 text-center">{dictionary.list_command.no_results}</p>
            )}
          </CommandList>
        </Command>
      </CommandDialog>
