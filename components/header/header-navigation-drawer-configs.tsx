...
export const HeaderNavigationDrawerConfigs = () => {
  ...
  const handleRedirectLanguageChange = (language: Language) => {
    ...
    const newParamsArray = paramsArray.map((param, index) => (index === 1 ? language : param))

    const newPathname = newParamsArray.join('/')
    replace(newPathname)
  }

  const currentLanguageOption = SUPPORTED_LANGUAGES.find((lang) => lang.value === language)
    ...
    <div>
      <div className="flex h-9 items-center justify-between p-2 text-sm">
        ...
        <div className="-mr-1.5 flex rounded-full border">
          {themes.map((i, index) => {
            const isActive = i === theme || (i === 'system' && theme === 'system' && resolvedTheme)
      ...
      <div className="flex items-center justify-between p-2 text-sm">
        ...
        <div>
          <Select onValueChange={(value) => handleRedirectLanguageChange(value as Language)}>
