      ...
      <NavigationMenuList>
        {items.map(({ label, items, icon: Icon, href }) => {
            ...
            return (
              <NavigationMenuItem key={label}>
                ...
                <NavigationMenuContent className="flex gap-4 p-4 md:w-[500px] lg:w-[700px]">
                  {href === '/movies' && <HeaderPopularMovie language={language} />}

                  {href === '/tv-series' && <HeaderPopularTvSerie language={language} />}
          ...
          return (
            <NavigationMenuItem
              key={label}
              className={cn(navigationMenuTriggerStyle(), getIsActive(href) && 'bg-muted')}
              asChild
            >
              <Link href={`/${language}${href}`} className="flex items-center gap-2">
                <Icon width={12} height={12} />
                {label}
              </Link>
            </NavigationMenuItem>
